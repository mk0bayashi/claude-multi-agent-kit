...
Conductorは **タスクを分析し、最適な実行方法を推薦** してから実行する。

### 判定フロー
1. **タスク分類**: 以下の3種に分類する
   - **(A) 単一エージェント** — 1つの専門エージェントで完結（パターン不要）
   - **(B) マルチエージェントパターン** — 複数エージェントの協調が必要 → パターン推薦
   - **(C) 要件不明** — 先にユーザーに質問して明確化
2. **パターン選定**: `.claude/task-routing.md` の判定マトリクスに基づき最適パターンを決定
3. **Subagent / Teams 選択**: Teams版が利用可能な5パターン(P2/P4/P5/P6/P11)では、デフォルトSubagent版、議論・対話が求められる場合のみTeams版を推薦
4. **AskUserQuestion で推薦を提示**: 推薦パターン + 理由 + 代替案をユーザーに提示
5. **確認後に実行**: ユーザーの選択に基づき、Skill ツールで該当コマンドを呼び出す

### ルーティング判定の詳細
`.claude/task-routing.md` を参照。判定マトリクス、単一エージェント判定、優先ルール等を記載。

### スラッシュコマンド直接指定
`/pattern-*` や `/team-*` を明示的に指定した場合は、推薦プロセスをスキップして即実行する。
...
- 契約書レビュー・NDAトリアージ・コンプライアンスチェックは **legal-reviewer** に委任
- 財務分析・仕訳・勘定照合・差異分析・SOXテストは **finance-analyst** に委任
- マーケティングコンテンツ・キャンペーン企画・SEO・競合分析は **marketing-strategist** に委任
...
> **自然言語でもOK**: スラッシュコマンドを知らなくても、タスクを自然言語で伝えれば
> Conductorが最適なパターンを推薦します（→「タスクルーティング」セクション参照）。
> コマンドを直接指定すれば、推薦プロセスをスキップして即実行します。
